class GraphicalPlayer extends Player {
  ...
  Move awaitMove() {
    ...
    if(showMove != null && showMove.getPiece() != null) {
      ArrayList<Move> moves = showMove.getPiece().getLegalMoves(board.pos, showMove.r, showMove.f);
        for(Move m : moves) {
          if(m == null) continue;
          ...
          pushStyle();
            noStroke();
            if(m.type == Type.TAKE) fill(71, 71, 254); // yellow
            else fill(148, 244, 148); // green
            circle(v.x, v.y, 15);
          popStyle();
      }
    }
